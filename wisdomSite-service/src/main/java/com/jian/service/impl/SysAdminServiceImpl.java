...
import com.jian.pojo.RoutesMeta;
import com.jian.pojo.RoutesPojo;
...
public class SysAdminServiceImpl implements SysAdminService {
	...
	public ResultUtil getUserInfo() {
		...
		SysRoleMenusExample example_ = new SysRoleMenusExample();
		com.jian.entity.SysRoleMenusExample.Criteria criteria_ = example_.createCriteria();
		criteria_.andRoleIdEqualTo(admin.getRoleId());
		List<SysRoleMenus> list = sysRoleMenusMapper.selectByExample(example_);
		List<Integer> integers = new ArrayList<>();
		for (SysRoleMenus s : list) {
			integers.add(s.getMenuId());
		}
		...
		List<RoutesPojo> routes = new ArrayList<>();
		for (SysMenus sm : menus) {
			if (sm.getParentId() == 0) {
				RoutesPojo pojo = new RoutesPojo();
				RoutesMeta meta = new RoutesMeta();
				pojo.setPath(sm.getHref());
				pojo.setComponent("#");
				meta.setTitle(sm.getMetaTitle());
				meta.setIcon(sm.getMetaIcon());
				pojo.setMeta(meta);
				List<RoutesPojo> childrens = new ArrayList<>();
				for (SysMenus sm2 : menus) {
					RoutesPojo pojo2 = new RoutesPojo();
					if (sm2.getParentId() == sm.getMenuId()) {
						RoutesMeta meta2 = new RoutesMeta();
						pojo2.setPath(sm2.getMetaTitle());
						pojo2.setComponent(sm2.getPath());
						meta2.setTitle(sm2.getMetaTitle());
						List<String> role = new ArrayList<>();
						for (SysMenus sm3 : menus) {
							if (sm3.getParentId() == sm2.getMenuId()) {
								role.add(sm3.getPermission());
							}
						}
						meta2.setRole(role);
						pojo2.setMeta(meta2);
						pojo2.setName(pojo2.getPath());
						childrens.add(pojo2);
					}

				}
				pojo.setChildren(childrens);
				routes.add(pojo);
			}
		}
	...
	public ResultUtil getUserInfo() {
		...
		if(StringUtils.equals(belongid, admin.getBelongId())){
			menus  = sysMenusMapper.selectByExample(example);
		}else{
		SysRoleMenusExample example_ = new SysRoleMenusExample();
		com.jian.entity.SysRoleMenusExample.Criteria criteria_ = example_.createCriteria();
		criteria_.andRoleIdEqualTo(admin.getRoleId());
		...
		for(SysMenus sm : menus){
			strings.add(sm.getPermission());
		}
		ResultUtil util = new ResultUtil();
		util.setCode(0);
		util.setData(strings);
		return util;
	}
	@Override
